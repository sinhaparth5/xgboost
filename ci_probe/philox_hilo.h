 ...
 */
#ifndef XGBOOST_HILO_PHILOX_ENGINE_H_
#define XGBOOST_HILO_PHILOX_ENGINE_H_
...
class PhiloxEngine {
 ...
 private:
  ...
  [[nodiscard]] static result_type MulHi(result_type a, result_type b) {
    if constexpr (2 * w <= 64) {
      // The whole 2w-bit product fits one native multiply. UMul64 below exists
      // for w == 64, where no such multiply is portable.
      auto const product = static_cast<std::uint64_t>(a) * static_cast<std::uint64_t>(b);
      return static_cast<result_type>((product >> w) & kMax64);
    } else {
  ...
  static void MulHiLo(result_type a, result_type b, result_type* p_hi, result_type* p_lo) {
    if constexpr (2 * w <= 64) {
      auto const product = static_cast<std::uint64_t>(a) * static_cast<std::uint64_t>(b);
      *p_hi = static_cast<result_type>((product >> w) & kMax64);
      *p_lo = static_cast<result_type>(product & kMax64);
    } else {
      std::uint64_t hi = 0, lo = 0;
      detail::UMul64(static_cast<std::uint64_t>(a), static_cast<std::uint64_t>(b), &hi, &lo);
      if constexpr (w == 64) {
        *p_hi = static_cast<result_type>(hi);
      } else {
        *p_hi = static_cast<result_type>(((hi << (64 - w)) | (lo >> w)) & kMax64);
      }
      *p_lo = static_cast<result_type>(lo & kMax64);
    }
  }
    ...
    for (std::size_t q = 0; q < r; ++q) {
      ...
      for (std::size_t k = 0; k < n / 2; ++k) {
        ...
        auto key = static_cast<result_type>(
            ...
            kMax64);
        result_type mul_hi = 0, mul_lo = 0;
        MulHiLo(v[2 * k], multipliers[k], &mul_hi, &mul_lo);
        state[2 * k]     = static_cast<result_type>(mul_hi ^ key ^ v[2 * k + 1]);
        state[2 * k + 1] = mul_lo;
      }
    }
    this->y_ = state;
...
#endif  // XGBOOST_HILO_PHILOX_ENGINE_H_
